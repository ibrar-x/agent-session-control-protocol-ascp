...
function hydrateTrustedDevice(row) {
    return {
        ...
        kdf: row.kdf,
        kdfParams: parseJson(row.kdf_params_json)
    };
}
export function createTrustStore(database) {
  ...
  `);
    return {
        getDevice(deviceId) {
            const row = selectDevice.get(deviceId);
            return row === undefined ? null : hydrateTrustedDevice(row);
        },
        listDevices() {
            return selectDevices.all().map(hydrateTrustedDevice);
        },
        upsertDevice(device) {
